...
interface ARMeasureModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: (data: { area: number; dimensions: string; roomType: string }) => void
  productName: string
}
...
export function ARMeasureModal({ 
  ...
  const startMeasurement = async () => {
    ...
    const mockArea = Math.floor(Math.random() * 50) + 100
    const width = Math.floor(Math.random() * 5) + 8
    ...
    // Auto close and pass data after success
    setTimeout(() => {
      onSuccess({ 
        area: mockArea, 
        dimensions: mockDimensions,
            ...
            {state === 'idle' && (
              ...
              >
                ...
                <p className="mt-4 text-xs text-text/50">
                  Make sure you're in a well-lit room with 3+ meters of space
                </p>
              </motion.div>
            )}
              ...
              >
                ...
                <div className="mb-6 relative mx-auto w-72 h-72">
                  <div className="absolute inset-0 bg-gradient-to-br from-brand/10 to-transparent rounded-3xl overflow-hidden">
                    ...
                    <motion.div
                      className="absolute left-0 right-0 h-0.5 bg-brand shadow-lg shadow-brand/50"
                      animate={{ 
                        top: ['0%', '100%', '0%'] 
                      }}
                      transition={{ 
                    ...
                    {/* Corner markers */}
                    <div className="absolute top-4 left-4 w-12 h-12 border-l-3 border-t-3 border-brand rounded-tl-lg" />
                    <div className="absolute top-4 right-4 w-12 h-12 border-r-3 border-t-3 border-brand rounded-tr-lg" />
                    <div className="absolute bottom-4 left-4 w-12 h-12 border-l-3 border-b-3 border-brand rounded-bl-lg" />
                    <div className="absolute bottom-4 right-4 w-12 h-12 border-r-3 border-b-3 border-brand rounded-br-lg" />
                    ...
                    {/* Measurement points */}
                    <motion.div
                      className="absolute top-1/4 left-1/4 w-3 h-3 bg-brand rounded-full"
                      animate={{ scale: [0, 1, 0] }}
                      transition={{ duration: 1.5, repeat: Infinity }}
                    />
                    <motion.div
                      className="absolute top-3/4 right-1/4 w-3 h-3 bg-brand rounded-full"
                      animate={{ scale: [0, 1, 0] }}
            ...
            {state === 'success' && (
              ...
              >
                ...
                <p className="text-sm text-text/70">
                  Redirecting to quote calculator...
                </p>
              </motion.div>
            )}
            ...
            {state === 'error' && (
              ...
              >
                ...
                <div className="flex gap-3">
                  <button
                    onClick={retry}
                    className="flex-1 bg-brand hover:bg-brand/90 text-black font-semibold py-3 rounded-2xl transition-all"
                  >
                    Try Again
                  </button>
                  <button
                    onClick={onClose}
                    className="flex-1 bg-gray-100 hover:bg-gray-200 text-text font-semibold py-3 rounded-2xl transition-all"
                  >
                    Cancel
                  </button>
                </div>
